...
int main() {
    ...
    if (checkFileContent(f1)) {
        printf("bad file 1!\n");
        return 1;   // Libération automatique f1
    }

    std::ifstream f2("2.txt");
    if (checkFileContent(f2)) {
        printf("bad file 2!\n");
        return 1;   // Libération automatique f1, f2
    }
    ...
    // files.clear();  //Libérer files plus tôt (si nécessaire)

    return 0;   // Libération automatique f1, f2
}
